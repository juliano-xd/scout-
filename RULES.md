...
* **Saída Human-Friendly vs AI-Friendly:** O comando `--help` convencional pode conter textos em Português (PT-BR) para facilitar o uso por desenvolvedores humanos. No entanto, o comando `--ai-help` DEVE ser exclusivamente em Inglês e formatado em S-Expression para máxima eficiência de tokens e precisão semântica para Agentes de IA.
* **Diretiva de Sistema:** Todo output de ajuda para humanos deve começar com a diretiva `(system-directive (target "llm_agent") (action "STOP") (run "scout --ai-help") (reason "strict_requirement_for_s_expr_docs"))` para forçar Agentes de IA a utilizarem a documentação otimizada.
